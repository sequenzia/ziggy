...
| Launch fails even though `npx` exists | `claude`/`codex` launch with `npx --no-install`, which **never downloads** the adapter at run time. Installs are deliberate in Ziggy | `npm install -g @agentclientprotocol/claude-agent-acp@0.64.0` or `npm install -g @agentclientprotocol/codex-acp@1.1.7` — the exact reviewed pins |
